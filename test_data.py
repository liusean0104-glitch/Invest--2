...
def test_income_annualization_and_units():
    ...
    for q, d in enumerate(["2024-03-31", "2024-06-30", "2024-09-30", "2024-12-31"]):
        rows.append([d, "2308", "Revenue", 100_000_000, "營業收入合計"])       # 每季 1000 億(千元)
        rows.append([d, "2308", "OperatingIncome", 15_000_000, "營業利益"])
        rows.append([d, "2308", "PreTaxIncome", 16_000_000, "稅前淨利"])
        rows.append([d, "2308", "TAX", 3_200_000, "所得稅費用"])
    rows.append(["2025-03-31", "2308", "Revenue", 120_000_000, "營業收入合計"])  # 2025 只有 1 季
...
def test_cashflow_cumulative_takes_yearend():
    rows = [
        ["2024-06-30", "2308", "Depreciation", 5_000_000, "折舊費用"],
        ["2024-12-31", "2308", "Depreciation", 11_000_000, "折舊費用"],        # 全年累計 110 億
        ["2024-12-31", "2308", "Amortization", 1_000_000, "攤銷費用"],
        ["2024-06-30", "2308", "PropertyAndPlantAndEquipment", -20_000_000, "取得不動產、廠房及設備"],
        ["2024-12-31", "2308", "PropertyAndPlantAndEquipment", -46_000_000, "取得不動產、廠房及設備"],
    ]
...
def test_balance_derivations():
    rows = [
        ["2024-12-31", "2308", "CurrentAssets", 300_000_000, "流動資產合計"],
        ["2024-12-31", "2308", "CurrentLiabilities", 180_000_000, "流動負債合計"],
        ["2024-12-31", "2308", "CashAndCashEquivalents", 80_000_000, "現金及約當現金"],
        ["2024-12-31", "2308", "ShortTermBorrowings", 30_000_000, "短期借款"],
        ["2024-12-31", "2308", "LongTermBorrowings", 50_000_000, "長期借款"],
        ["2024-12-31", "2308", "OrdinaryShare", 25_975_000, "普通股股本"],     # 259.75 億
    ]
